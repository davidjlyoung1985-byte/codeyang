...
export function executeToolSearch(query: string): string {
  ...
  const matches = schemas.filter((t) => t.name.toLowerCase().includes(kw) || t.description.toLowerCase().includes(kw));

  if (matches.length === 0) return `No tools found matching: ${query}`;

  const lines: string[] = [`Tools matching "${query}" (${matches.length}):`, ''];
