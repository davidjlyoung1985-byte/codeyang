...
const CONFIG_DIR = getCodeyangHome();
const CONFIG_FILE = codeyangPath('config.json');
...
async function safeRename(src: string, dest: string): Promise<void> {
  try {
    ...
    if (err instanceof Error && 'code' in err && (err as NodeJS.ErrnoException).code === 'EXDEV') {
      ...
      await unlink(src).catch(() => logger.warn('Failed to clean up temp file during atomic write'));
    } else {
      throw err;
    }
  }
...
export async function reloadConfig(): Promise<void> {
  ...
  reloadingPromise = (async () => {
    try {
      ...
      _configVersion++;
      logger.info(`Configuration reloaded (v${_configVersion})`);
    } catch (err) {
