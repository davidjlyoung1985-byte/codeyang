...
export function validateFilePath(path: string, allowedRoot?: string): boolean {
  ...
  // Compare on a POSIX-normalized form so the check works regardless of the
  // host separator (e.g. `normalize('/etc')` yields `\etc` on Windows).
  const posixPath = normalizedPath.replace(/\\/g, '/');
  const dangerousDirs = ['/etc', '/sys', '/proc', '/dev', '/root'];

  for (const dir of dangerousDirs) {
    if (posixPath.startsWith(dir)) {
      return false;
    }
  }
...
export function validateUrl(url: string): boolean {
  ...
  try {
    ...
    // Block IPv6 localhost. `URL.hostname` keeps the square brackets for IPv6
    // literals (e.g. `http://[::1]` -> `[::1]`), so strip them first.
    const bareHostname = hostname.replace(/^\[|\]$/g, '');
    if (bareHostname === '::1' || bareHostname.includes('0000:0000:0000:0000:0000:0000:0000:0001')) {
      return false;
    }
...
export function redactSensitiveData(obj: unknown): unknown {
  ...
  const redacted: Record<string, unknown> = Array.isArray(obj) ? ([] as unknown as Record<string, unknown>) : {};
