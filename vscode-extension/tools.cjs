    ...
    FormData4.prototype._lengthRetriever = function(value, callback) {
      if (hasOwn(value, "fd")) {
        if (value.end != void 0 && value.end != Infinity && value.start != void 0) {
          ...
          fs3.stat(value.path, function(err, stat5) {
            if (err) {
              callback(err);
              return;
            }
  ...
  "node_modules/follow-redirects/index.js"(exports2, module2) {
    ...
    function parseUrl2(input) {
      ...
      if (useNativeURL) {
        ...
        parsed = validateUrl2(url2.parse(input));
...
__export(shared_exports, {
  ...
  resolveSafePathAsync: () => resolveSafePathAsync
});
...
function netError(url2, detail, isTimeout = false) {
  return toolErrorWithActions({
    severity: "error",
    context: "Network",
    message: `${url2}: ${detail}`,
...
async function readFileWithRetry(filePath, retries = MAX_READ_RETRIES) {
...
var SlidingWindowTracker = class {
  ...
  check() {
    ...
    if (this.timestamps.length >= this.config.maxCalls) {
      ...
      throw new Error(
        `[RATE LIMIT] Too many operations. Limit: ${this.config.maxCalls} calls per ${this.config.windowMs / 1e3}s. Wait ~${Math.ceil(waitMs / 1e3)}s before retrying.`
      );
    }
...
function isProtectedFile(filePath) {
  ...
  if (PROTECTED_FILES.includes(basename3)) {
    return true;
  }
  for (const pattern of PROTECTED_FILES) {
    if (pattern.includes("*")) {
      const regex = new RegExp("^" + pattern.replace(/\*/g, ".*") + "$");
      if (regex.test(basename3)) {
        return true;
      }
    }
  }
...
async function executeWrite(filePath, content) {
  ...
  if (isProtectedFile(filePath)) {
    ...
    throw new Error(
      toolError(
        "Write",
        `Cannot write to protected file: ${basename3}`,
  ...
  const dir = (0, import_node_path.dirname)(resolved);
  await (0, import_promises2.mkdir)(dir, { recursive: true });
  const tmpPath = (0, import_node_path.resolve)(dir, `.tmp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
  try {
    await (0, import_promises2.writeFile)(tmpPath, content, "utf-8");
    await (0, import_promises2.rename)(tmpPath, resolved);
  } catch (err) {
    try {
      await import("fs/promises").then((m) => m.unlink(tmpPath));
    } catch {
    }
    throw err;
  }
...
// src/tools/EditTool.ts
async function saveDiskBackup(filePath, content) {
  const undoDir = (0, import_node_path2.join)(process.cwd(), ".codeyang", "undo");
  await (0, import_promises3.mkdir)(undoDir, { recursive: true });
  const safeName = filePath.replace(/[\\/:*?"<>|]/g, "_") + "." + Date.now() + ".backup";
  const backupPath = (0, import_node_path2.join)(undoDir, safeName);
  await (0, import_promises3.writeFile)(backupPath, content, "utf-8");
}
var editLocks = /* @__PURE__ */ new Map();
...
var MAX_GLOB_PATTERN_LENGTH = 500;
...
async function executeGlob(pattern, root) {
  try {
    ...
    const base = root ? (0, import_node_path3.isAbsolute)(root) ? root : (0, import_node_path3.join)(process.cwd(), root) : process.cwd();
    const results = [];
    ...
    async function walk(dir) {
      ...
      for (const entry of entries) {
        ...
        const rel = (0, import_node_path3.relative)(base, full).replace(/\\/g, "/");
        if (regex.test(rel)) {
          results.push(rel);
        }
...
async function executeGrep(pattern, include, path4, contextLines = 0) {
  const base = path4 ? (0, import_node_path4.isAbsolute)(path4) ? path4 : (0, import_node_path4.join)(process.cwd(), path4) : process.cwd();
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (matched) {
          const relPath = (0, import_node_path4.relative)(base, full).replace(/\\/g, "/");
...
function getCodeyangHome() {
  return process.env.CODEYANG_HOME || (0, import_node_path5.join)((0, import_node_os.homedir)(), ".codeyang");
}
function codeyangPath(...segments) {
  return (0, import_node_path5.join)(getCodeyangHome(), ...segments);
}

// src/utils/todoStore.ts
var DIR = codeyangPath();
var FILE = (0, import_node_path6.join)(DIR, "todos.json");
...
var package_default = {
  ...
  repository: {
    type: "git",
    url: "git+https://github.com/davidjlyoung1985-byte/codeyang.git"
  },
  homepage: "https://github.com/davidjlyoung1985-byte/codeyang#readme",
  bugs: {
    url: "https://github.com/davidjlyoung1985-byte/codeyang/issues"
  },
  scripts: {
    build: "tsup --config tsup.config.ts",
    dev: "tsup src/index.ts --format esm --clean --sourcemap --watch --outDir dist",
    start: "node dist/index.js",
    "web:server": "tsx web/server/index.ts",
    "web:client": "vite",
    ...
    "generate-docs": "npx tsx src/tools/auto-docs.ts",
...
if (axios_default.defaults) {
  axios_default.defaults.timeout = 3e4;
}
...
async function validateUrl(url2) {
  if (url2.length > MAX_URL_LENGTH) {
    return `URL too long (${url2.length} chars, max ${MAX_URL_LENGTH})`;
  }
  let parsed;
  try {
    parsed = new URL(url2);
...
var USER_AGENT = `CodeYang-Agent/${VERSION}`;

// src/tools/WebFetchTool.ts
var MAX_REDIRECTS = 5;
var MAX_BODY_SIZE = 10 * 1024 * 1024;
async function executeWebFetch(url2, format) {
  ...
  try {
    ...
    if (!["http:", "https:"].includes(parsed.protocol)) {
      throw new Error(netError(url2, `Unsupported protocol: ${parsed.protocol}`));
    }
  } catch {
    throw new Error(netError(url2, "Invalid URL"));
  }
  const outputFormat = format === "html" ? "html" : "text";
  return fetchWithRedirectLimit(url2, outputFormat, 0);
}
async function fetchWithRedirectLimit(url2, outputFormat, redirectCount) {
  if (redirectCount > MAX_REDIRECTS) {
    throw new Error(
      toolError(
        "Network",
        `Too many redirects (max ${MAX_REDIRECTS}): ${url2}`,
  ...
  try {
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      ...
      if (!location) {
        throw new Error(netError(url2, `Redirect ${response.status} without Location header`));
      }
      const nextUrl = new URL(location, url2).href;
      const validationError = await validateUrl(nextUrl);
      if (validationError) {
        throw new Error(netError(url2, `Redirect blocked: ${validationError}`));
      }
      return fetchWithRedirectLimit(nextUrl, outputFormat, redirectCount + 1);
    }
    if (!response.ok) {
      throw new Error(netError(url2, `HTTP ${response.status}: ${response.statusText}`));
    }
    ...
    if (isHtml && outputFormat === "text") {
      let text = html.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "").replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "").replace(/<noscript[^>]*>[\s\S]*?<\/noscript>/gi, "").replace(/<!--[\s\S]*?-->/g, "").replace(/<\[CDATA\[[\s\S]*?\]\]>/g, "").replace(/<\/(p|div|h[1-6]|section|article|header|footer|nav|aside|blockquote|table|tr)>/gi, "\n").replace(/<(br|hr|li|dd|dt)[^>]*>/gi, "\n").replace(/<\/?(ul|ol|li|dl|dt|dd)[^>]*>/gi, "\n").replace(/<a[^>]*href="([^"]*)"[^>]*>([\s\S]*?)<\/a>/gi, "$2 ($1)").replace(/<a[^>]*href='([^']*)'[^>]*>([\s\S]*?)<\/a>/gi, "$2 ($1)").replace(/<img[^>]*alt="([^"]*)"[^>]*>/gi, "$1").replace(/<img[^>]*alt='([^']*)'[^>]*>/gi, "$1");
      let prev = "";
      while (text !== prev) {
        prev = text;
        text = text.replace(/<[^>]*>/g, "");
      }
      text = text.replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&nbsp;/g, " ").replace(/&#(\d+);/g, (_, code) => String.fromCharCode(Number(code))).replace(/&#x([0-9a-fA-F]+);/g, (_, code) => String.fromCharCode(parseInt(code, 16)));
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new Error(
        toolError("Network", `Request timed out after 15s: ${url2}`, "The server may be slow or unreachable.")
      );
    }
    throw err;
  }
  ...
  try {
    ...
    for (const entry of dirEntries) {
      ...
      if (entry.isDirectory()) {
        ...
        entries.push((0, import_node_path7.relative)(base, full));
      }
    }
  } catch {
  }
  ...
  if (searchContent && results.length < maxResults) {
    try {
      ...
      const grepOut = await Promise.race([
        ...
        new Promise(
          (_, reject) => setTimeout(() => reject(new Error("Search timed out after 30s")), SEARCH_TIMEOUT)
        )
      ]);
...
function resolveSafePath(inputPath, cwd) {
  ...
  const resolved = (0, import_node_path8.resolve)(base, inputPath);
  const sandbox = process.env["CODEX_SANDBOX"];
  if (!sandbox) return resolved;
  let absSandbox = (0, import_node_path8.resolve)(sandbox);
  try {
  ...
  const sandboxSep = absSandbox.endsWith(import_node_path8.sep) ? absSandbox : absSandbox + import_node_path8.sep;
...
async function resolveSafePathAsync(inputPath, cwd) {
  const base = cwd || process.cwd();
  const resolved = (0, import_node_path8.resolve)(base, inputPath);
  const sandbox = process.env["CODEX_SANDBOX"];
  if (!sandbox) return resolved;
  let absSandbox = (0, import_node_path8.resolve)(sandbox);
  try {
    absSandbox = await (0, import_promises8.realpath)(absSandbox);
  } catch {
  }
  if (resolved === absSandbox) return resolved;
  let real = resolved;
  try {
    real = await (0, import_promises8.realpath)(resolved);
  } catch {
  }
  const sandboxSep = absSandbox.endsWith(import_node_path8.sep) ? absSandbox : absSandbox + import_node_path8.sep;
  const isInsideSandbox = real.startsWith(sandboxSep) || real === absSandbox;
